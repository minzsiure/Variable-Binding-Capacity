...
title_str = 'Original Reciprocal Project. Similarity between singular vectors and input/output assemblies'
title_str += "\nN=1000, k=100, p=0.1, beta=0.1, norm_every=5, recurr=True,"+\
                "\nlearning max_iter=50, noise q=0.01, r=0.9, test_iter=5."
outfilename = 'recip_2classes.pdf'
...
y1, z2, W_yy1, W_zz1, W_xy1, W_zy1, W_yz1 = B.reciprocal_project_stream(S1,0,1,2, max_iterations=50, return_weights_assembly=True)
y2, z2, W_yy2, W_zz2, W_xy2, W_zy2, W_yz2 = B.reciprocal_project_stream(S2,0,1,2, max_iterations=50, initialize=False, return_weights_assembly=True)

X1 = S1.sample_stimuli(100)
X2 = S2.sample_stimuli(100)

y_new_1, z_new_1 = B.create_assemblies_reciprocal_project(X1[:,0],0,1,2)
y_new_2, z_new_2 = B.create_assemblies_reciprocal_project(X2[:,0],0,1,2)
...
plt_limit = 400
fig, ((ax1, ax2), (ax3, ax4), (ax5, ax6), (ax7, ax8), (ax9, ax10), (ax11, ax12)) = plt.subplots(6, 2, sharex=True, sharey=True, figsize=(18,20))
fig.suptitle(title_str)
ax1.plot(sv_idxs[:plt_limit], Wxy_v_dist1[Wxy_v_sort_order[:plt_limit]], 'r--', label='Distance to Class 1')
ax1.plot(sv_idxs[:plt_limit], Wxy_v_dist2[Wxy_v_sort_order[:plt_limit]], 'b:', label='Distance to Class 2')
ax1.legend(loc='lower right')
ax1.set_ylabel('$W_{XC}$', fontsize=16)
ax1.set_title('$d_H(v, x)$')
ax2.plot(sv_idxs[:plt_limit], Wxy_u_dist1[Wxy_u_sort_order[:plt_limit]], 'r--')
ax2.plot(sv_idxs[:plt_limit], Wxy_u_dist2[Wxy_u_sort_order[:plt_limit]], 'b:')
ax2.set_title('$d_H(u, c)$')

ax3.plot(sv_idxs[:plt_limit], Wzy_v_dist1[Wzy_v_sort_order[:plt_limit]], 'r--')
ax3.plot(sv_idxs[:plt_limit], Wzy_v_dist2[Wzy_v_sort_order[:plt_limit]], 'b:')
ax3.set_ylabel('$W_{VC}$', fontsize=16)
ax3.set_title('$d_H(v, v)$')
ax4.plot(sv_idxs[:plt_limit], Wzy_u_dist1[Wzy_u_sort_order[:plt_limit]], 'r--')
ax4.plot(sv_idxs[:plt_limit], Wzy_u_dist2[Wzy_u_sort_order[:plt_limit]], 'b:')
ax4.set_title('$d_H(u, c)$')

# ax5.plot(sv_idxs[:plt_limit], Wxz_v_dist1[Wxz_v_sort_order[:plt_limit]], 'r--')
# ax5.plot(sv_idxs[:plt_limit], Wxz_v_dist2[Wxz_v_sort_order[:plt_limit]], 'b:')
ax5.set_ylabel('$W_{XV}$', fontsize=16)
# ax5.set_title('$d_H(v, x)$')
# ax6.plot(sv_idxs[:plt_limit], Wxz_u_dist1[Wxz_u_sort_order[:plt_limit]], 'r--')
# ax6.plot(sv_idxs[:plt_limit], Wxz_u_dist2[Wxz_u_sort_order[:plt_limit]], 'b:')
# ax6.set_title('$d_H(u, v)$')

ax7.plot(sv_idxs[:plt_limit], Wyz_v_dist1[Wyz_v_sort_order[:plt_limit]], 'r--')
ax7.plot(sv_idxs[:plt_limit], Wyz_v_dist2[Wyz_v_sort_order[:plt_limit]], 'b:')
ax7.set_ylabel('$W_{CV}$', fontsize=16)
ax7.set_title('$d_H(v, c)$')
ax8.plot(sv_idxs[:plt_limit], Wyz_u_dist1[Wyz_u_sort_order[:plt_limit]], 'r--')
ax8.plot(sv_idxs[:plt_limit], Wyz_u_dist2[Wyz_u_sort_order[:plt_limit]], 'b:')
ax8.set_title('$d_H(u, v)$')

ax9.plot(sv_idxs[:plt_limit], Wyy_v_dist1[Wyy_v_sort_order[:plt_limit]], 'r--')
ax9.plot(sv_idxs[:plt_limit], Wyy_v_dist2[Wyy_v_sort_order[:plt_limit]], 'b:')
ax9.set_ylabel('$W_{CC}$', fontsize=16)
ax9.set_title('$d_H(v, c)$')
ax10.plot(sv_idxs[:plt_limit], Wyy_u_dist1[Wyy_u_sort_order[:plt_limit]], 'r--')
ax10.plot(sv_idxs[:plt_limit], Wyy_u_dist2[Wyy_u_sort_order[:plt_limit]], 'b:')
ax10.set_title('$d_H(u, c)$')

ax11.plot(sv_idxs[:plt_limit], Wzz_v_dist1[Wzz_v_sort_order[:plt_limit]], 'r--')
ax11.plot(sv_idxs[:plt_limit], Wzz_v_dist2[Wzz_v_sort_order[:plt_limit]], 'b:')
ax11.set_ylabel('$W_{VV}$', fontsize=16)
ax11.set_title('$d_H(v, z)$')
ax11.set_xlabel('Singular Vector Index')
ax12.plot(sv_idxs[:plt_limit], Wzz_u_dist1[Wzz_u_sort_order[:plt_limit]], 'r--')
ax12.plot(sv_idxs[:plt_limit], Wzz_u_dist2[Wzz_u_sort_order[:plt_limit]], 'b:')
ax12.set_title('$d_H(u, z)$')
ax12.set_xlabel('Singular Vector Index')
