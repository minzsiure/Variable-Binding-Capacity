...
FONTSIZE = 7
LABELSIZE = 4
fig, ((ax5, ax6), (ax7, ax8), (ax3, ax4), (ax11, ax12)) = plt.subplots(4, 2, sharex=True, sharey=True)
fig.set_size_inches(4, 5)
# fig.suptitle(title_str, fontsize=FONTSIZE)

ax5.plot(sv_idxs[:plt_limit], Wxz_v_dist1[Wxz_v_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1, label='Distance to Class 1')
ax5.plot(sv_idxs[:plt_limit], Wxz_v_dist2[Wxz_v_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1, label='Distance to Class 2')
...
# ax5.set_xticks([])
ax6.plot(sv_idxs[:plt_limit], Wxz_u_dist1[Wxz_u_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax6.plot(sv_idxs[:plt_limit], Wxz_u_dist2[Wxz_u_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
ax6.set_title('$d_H(left\ SV, v)$', fontsize=FONTSIZE)
# ax6.set_xticks([])
# ax6.set_yticks([])

ax7.plot(sv_idxs[:plt_limit], Wyz_v_dist1[Wyz_v_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax7.plot(sv_idxs[:plt_limit], Wyz_v_dist2[Wyz_v_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
ax7.set_ylabel('$W_{CV}$', fontsize=FONTSIZE)
ax7.set_title('$d_H(right\ SV, c)$', fontsize=FONTSIZE)
# ax7.set_xticks([])
ax8.plot(sv_idxs[:plt_limit], Wyz_u_dist1[Wyz_u_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax8.plot(sv_idxs[:plt_limit], Wyz_u_dist2[Wyz_u_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
ax8.set_title('$d_H(left\ SV, v)$', fontsize=FONTSIZE)
# ax8.set_xticks([])
# ax8.set_yticks([])

ax3.plot(sv_idxs[:plt_limit], Wzy_v_dist1[Wzy_v_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax3.plot(sv_idxs[:plt_limit], Wzy_v_dist2[Wzy_v_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
ax3.set_ylabel('$W_{VC}$', fontsize=FONTSIZE)
ax3.set_title('$d_H(right\ SV, v)$', fontsize=FONTSIZE)
# ax3.set_xticks([])
ax4.plot(sv_idxs[:plt_limit], Wzy_u_dist1[Wzy_u_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax4.plot(sv_idxs[:plt_limit], Wzy_u_dist2[Wzy_u_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
ax4.set_title('$d_H(left\ SV, c)$', fontsize=FONTSIZE)
# ax4.set_xticks([])
# ax4.set_yticks([])

ax11.plot(sv_idxs[:plt_limit], Wzz_v_dist1[Wzz_v_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax11.plot(sv_idxs[:plt_limit], Wzz_v_dist2[Wzz_v_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
...
ax12.plot(sv_idxs[:plt_limit], Wzz_u_dist1[Wzz_u_sort_order[:plt_limit]], color='#D62628', ls='--', lw=1)
ax12.plot(sv_idxs[:plt_limit], Wzz_u_dist2[Wzz_u_sort_order[:plt_limit]], color='#1E76B4', ls=':', lw=1)
ax12.set_title('$d_H(left\ SV, v)$', fontsize=FONTSIZE)
ax12.set_xlabel('Singular Vector Index', fontsize=FONTSIZE)
# ax12.set_yticks([])

mpl.rcParams['axes.linewidth'] = 0.01 # set subplot border thickness value globally
fig.tight_layout()
for ax in fig.get_axes():
    ax.label_outer() # remove inner labels
    ax.tick_params(axis='both', labelsize=LABELSIZE) # set tick label font size
